...
@injectable()
export default class PickPointController {
    constructor(
        @inject(TYPES.PickPointService)
        private _pickPointService: PickPointService
    ) {}
    ...
    public async getDataTable(req: Request, res: Response): Promise<Response> {
        ...
        const data = await this._pickPointService.getDataTable(
            validatedReq.data
        );
        return res.json({
            message: "success",
